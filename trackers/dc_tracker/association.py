...
def associate_4_points_with_score_with_depth(
    ...
    total_cost = -(iou_matrix + angle_diff_cost - score_dif * args.TCM_first_step_weight) + depth_weight * depth_cost
    print("cost_no_depth")
    print( -(iou_matrix + angle_diff_cost - score_dif * args.TCM_first_step_weight))
    print("total_cost")
    print(total_cost)
...
##深度レベルの分割
def split_by_depth_indices(num_objects, depths, depth_levels):
    """
    オブジェクトを深度レベルに基づいて分割します。

    num_objects: オブジェクトの数
    depths: オブジェクトに対応する深度値の配列
    depth_levels: 深度レベルの配列
    """
    subsets = [[] for _ in range(len(depth_levels) - 1)]
    for idx in range(num_objects):
        depth = depths[idx]
        # 深度レベルのインデックスを取得
        level_idx = np.searchsorted(depth_levels, depth, side='right') - 1
        level_idx = min(max(level_idx, 0), len(depth_levels) - 2)
        subsets[level_idx].append(idx)
    return subsets
