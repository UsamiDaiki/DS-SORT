...
def make_parser():
    ...
    # Depth-Anything V2 関連
    parser.add_argument("--with_depth", action='store_true', help="enable depth estimation")
    parser.add_argument("--depth_encoder", type=str, choices=['vitl', 'vitb', 'vits', 'vitg'], default='vitl', help="Depth-Anything V2 encoder type")
    parser.add_argument("--depth_input_size", type=int, default=518, help="Depth-Anything V2 input size")
    parser.add_argument("--depth_weights", type=str, default='checkpoints/depth_anything_v2_vitl.pth', help="Path to Depth-Anything V2 weights")
    parser.add_argument("--save_depth", action='store_true', help="save depth maps")
    parser.add_argument("--visualize_depth", action='store_true', help="overlay depth maps on tracking results")

    # 頭部検出関連の引数を追加
    parser.add_argument("--with_head_detection", action='store_true', help="enable head detection")
    parser.add_argument("--head_exp", type=str, default=None, help="path to head detection experiment file (e.g., exps/example/custom/yolox_head_crowdhuman.py)")
    parser.add_argument("--head_weights", type=str, default=None, help="path to head detection model weights (e.g., checkpoints/best_head.pth)")


    parser.add_argument("--with_sam", action='store_true', help="enable sam segmentation")
    parser.add_argument(
        "--sam_model_type",
        type=str,
        default="vit_h",
        help="Type of SAM model to use. e.g., 'vit_h', 'vit_l', 'vit_b'."
    )
    parser.add_argument(
        "--sam_checkpoint_path",
        type=str,
