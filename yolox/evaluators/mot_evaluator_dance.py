...
class MOTEvaluator:
    ...
    ## with_depth=Falseを追加
    def __init__(
        self, args, dataloader, img_size, confthre, nmsthre, num_classes, with_depth=False, with_sam=False):
        """
        Args:
        ...
        if self.with_depth:
            ...
            depth_weights = args.depth_weights  # 重みファイルのパス
            self.depth_estimator.load_state_dict(torch.load(depth_weights, map_location='cuda'))
    ...
    def evaluate_ex_sort(
        ...
        for cur_iter, (imgs, _, info_imgs, ids, raw_image) in enumerate(    # [hgx0411] add raw_image for FastReID
            ...
            with torch.no_grad():
                ...
                ##Ex_Sort_1に変更
                if frame_id == 1:
                    tracker = Dc_Sort(args, det_thresh=self.args.track_thresh, iou_threshold=self.args.iou_thresh,
                                     asso_func=self.args.asso, delta_t=self.args.deltat, inertia=self.args.inertia,
                                     use_byte=self.args.use_byte)
                    if len(results) != 0:
                        result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[video_id - 1]))
                ...
                ## 深度推定の実行
                if self.with_depth:
                    img = raw_image[0]  # バッチサイズが 1 の場合
                    #img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

                    # 画像をテンソルに変換
                    #img_tensor = torch.from_numpy(img_rgb).float().permute(2, 0, 1).unsqueeze(0).to('cuda') / 255.0


                    # 深度マップの推定
                    #depth_map = self.depth_estimator.infer_image(img_tensor)
                    ...
                    # print('Min value in img:', img.min())
                    ...
                    # 深度マップの推定（色空間の変換は不要）
                    depth_map = self.depth_estimator.infer_image(img)
                    ...
                    # 深度マップのリサイズ
                    depth_map = torch.nn.functional.interpolate(
                        depth_map.unsqueeze(0).unsqueeze(0),  # [H, W] -> [1, 1, H, W]
                        size=(img.shape[0], img.shape[1]),
                        mode='bilinear',
                        align_corners=False
            ...
            ## 深度マップを渡す
            # run tracking
            online_targets = tracker.update(outputs[0], info_imgs, self.img_size, depth=depth_map)
            online_tlwhs = []
            online_ids = []
            for t in online_targets:
                """
    ...
    def evaluate_seg_sort(
        ...
        for cur_iter, (imgs, _, info_imgs, ids, raw_image) in enumerate(    # [hgx0411] add raw_image for FastReID
            ...
            with torch.no_grad():
                ...
                ##Ex_Sort_1に変更
                if frame_id == 1:
                    tracker = Ds_Sort(args, det_thresh=self.args.track_thresh, iou_threshold=self.args.iou_thresh,
                                     asso_func=self.args.asso, delta_t=self.args.deltat, inertia=self.args.inertia,
                                     use_byte=self.args.use_byte)
                    if len(results) != 0:
                        result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[video_id - 1]))
                ...
                if self.with_depth:
                    ...
                    # 深度マップの推定（色空間の変換は不要）
                    depth_map = self.depth_estimator.infer_image(img, depth_input_size)
                    depth_map = torch.from_numpy(depth_map).float()
                ...
                if self.with_sam and outputs[0] is not None and outputs[0].shape[0] > 0:
                    ...
                    # 検出結果のバウンディングボックスを元スケールに戻す
                    bboxes = outputs[0][:, :4] / scale
                    # SAM の入力は RGB 画像なので、raw_image[0] を BGR→RGB 変換
                    image_rgb = cv2.cvtColor(raw_image[0].copy(), cv2.COLOR_BGR2RGB)
                    ...
                    for bbox in bboxes:
                        if torch.is_tensor(bbox):
                            ...
                            bbox_int = np.array(bbox).astype(np.int32).tolist()
                        input_box = np.array(bbox_int).reshape(1, -1)
                ...
                # --- セグメント処理終了 ---
                """
                # --- SAM 処理：スコア閾値により mask 生成する検出を限定 ---
                if self.with_sam and outputs[0] is not None and outputs[0].shape[0] > 0:
                    first_round_thresh = 0.7  # 閾値（例：0.7）
                    ...
                    # scale の計算（convert_to_coco_format で使用している方法に準ずる）
                    scale = min(
                        self.img_size[0] / float(info_imgs[0]),
                        self.img_size[1] / float(info_imgs[1])
                    )
                    masks = []
                    if valid_inds.sum() > 0:
                        bboxes_valid = outputs[0][valid_inds, :4] / scale
                        # SAM には RGB 画像が必要（raw_image[0] は BGR と仮定）
                        if isinstance(raw_image[0], torch.Tensor):
                            img = raw_image[0].cpu().numpy()
                        image_rgb = cv2.cvtColor(img.copy(), cv2.COLOR_BGR2RGB)
                        self.sam_predictor.set_image(image_rgb)
